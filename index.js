...
const createListElement = async () => {
  ...
  listItems.forEach((movie) => {
    ...
    for (let key in movie) {
      if (key === "name") {
        div_st2_name.appendChild(
          document.createTextNode(` name: ${movie[key]}`)
        );
        div_row_st2.appendChild(div_st2_name);
      }
      if (key === "season") {
        div_st2_season.appendChild(
          document.createTextNode(` season: ${movie[key]}`)
        );
        div_row_st2.appendChild(div_st2_season);
      }
      if (key === "number") {
        div_st2_episode.appendChild(
          document.createTextNode(` episode: ${movie[key]}`)
        );
      ...
      if (key === "summary") {
        ...
        div_st2_2_summary.appendChild(
          document.createTextNode(`summary: ${summary}`)
        );
